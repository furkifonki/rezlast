    ...
    <div className="pb-6 md:pb-8">
      ...
      <form onSubmit={handleSubmit} className="p-4 md:p-6 space-y-5 md:max-w-2xl md:mx-auto md:bg-white md:rounded-b-xl md:border md:border-t-0 md:border-[#e2e8f0]">
            ...
            <div className="flex flex-wrap gap-2">
              {slots.map((t) => (
                <button
                  ...
                  className={reservationTime === t ? 'px-4 py-2 rounded-xl text-sm font-medium bg-green-700 text-white' : 'px-4 py-2 rounded-xl text-sm font-medium bg-slate-50 text-slate-500 border border-slate-200'}
                >
                  {t.slice(0, 5)}
                </button>
              ))}
        ...
        {availableTables.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-[#64748b] mb-2">Masa (opsiyonel)</p>
            <select
              value={tableId ?? ''}
        ...
        <div>
          <label className="block text-xs font-semibold text-[#64748b] mb-1.5">Özel istekler (opsiyonel)</label>
          <textarea
